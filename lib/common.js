...
function padding(length, timestamp) {
	const value = { level: -1, date: timestamp, count: 0 };
...
export async function getGitHubContributions(user, to) {
	...
	const begin = commits.at(0).date;
	const end = commits.at(-1).date;

	const dayOfWeek = new Date(begin).getDay();
	commits.unshift(...padding(dayOfWeek, 0));

	return { begin, end, commits };
}
